...
\begin{enumerate}
	\item[(1)] 정리 5.8.
	\item[(2)] 공사건이 아닌 두 사건 $A, B$가 서로 배반사건이면, 사건 $A, B$는 종속이다. 
\end{enumerate}
}
...
\thm{\textbf{(독립시행의 확률)} $1$회의 시행에서 사건 $A$가 일어날 확률을 $p$ 라 할 때, 이 시행을 $n$회 반복하는 독립시행에서 사건 $A$가 $r$번 일어날 확률은 다음과 같다. $${n\choose r} p^r(1-p)^{n-r} \quad \big(r=0, 1, \dots, n\big)$$}
