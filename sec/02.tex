...
\defn{변량 $x_i$가 \textbf{최댓값}(maximum)이면 모든 $j$에 대해 $x_i\geq x_j$ 이고, $x_i$가 \textbf{최솟값}(minimum)이면 모든 $j$에 대해 $x_i\leq x_j$ 이다. 최댓값에서 최솟값을 뺀 값을 \textbf{범위}(range)라 한다.}


\defn{$Q_3$에서 $Q_1$을 뺀 값을 \textbf{사분위수범위}(interquartile range $IQR$)로 정의한다. $$IQR = Q_3-Q_1$$}\\
위 통계량들을 표본에 대해서도 정의할 수 있다. 모집단으로부터 표본 $x_1, \dots, x_n$를 얻었다고 하고, 이를 순서대로 나열한 것을 $x_{(1)}, \dots, x_{(n)}$이라 하자.
\pagebreak